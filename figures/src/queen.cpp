#include "queen.h"
#include "board.h"
queen::queen(colors color,std::shared_ptr<Board> parent,int cost,figure_weight weight) : figure(color,figure_type::Queen,parent,cost,weight)
{
...
void queen::touch_attacking_squares(int how) {
    ...
    p.column = columns(static_cast<int>(p.column) - 1);
    p.row++;
    while(p.column >= columns::A && p.row <= 8 && (b->at(p)._figure == EMPTY_SQUARE || (b->at(p)._figure->name() == figure_type::King && at(p)._figure->get_color() != get_color()))) {
        if(this->get_color() == colors::White) {
            b->at(p).attacking_white_figures+= how;
        }
        else {
...
std::vector<position> queen::possible_squares() {
    ...
    if(p.column >= columns::A && p.row <= 8 && b->at(p)._figure != nullptr && b->at(p)._figure->get_color() != this->get_color()) {
        result.push_back(p);
    }
