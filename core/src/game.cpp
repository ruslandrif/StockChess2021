...
void Game::traverse_board(std::function<void(square&)> func_to_traverse) {
    if(!b) {
        LOG(ERROR) << "b is null";
        return;
    }
    for(int i = 0;i<8;++i) {
        for(int j = 0;j<8;++j) {
            func_to_traverse(b->board[i][j]);
        }
    }
}

square& Game::find_some_figure(std::function<bool(square&)> chech_figure) {
    for(int i = 0;i<8;++i) {
        for(int j = 0;j<8;++j) {
            if(chech_figure(b->board[i][j]))
                return b->board[i][j];
        }
    }
    square s;
...
bool Game::check_some_board_condition(std::function<bool(square&)> check_false) {
    for(int i = 0;i<8;++i) {
        for(int j = 0;j<8;++j) {
            if(check_false(b->board[i][j]))
                return false;
        }
    }
    return true;
...
{
    LOG(INFO) << "before create b";
    b = std::make_shared<Board>(this);
    LOG(INFO) << "After create b";
    b->hide();
    ...
    connect(play_mode_setting.get(),static_cast<void(QComboBox::*)(const QString &)>(&QComboBox::textActivated),this,[this](const QString& s){
        if(s == "play against yourself") Game::st = state::BY_MYSELF;
        if(s == "play with computer") Game::st = state::STOCKFISH;
    });
    connect(hard_mode_setting.get(),static_cast<void(QComboBox::*)(const QString &)>(&QComboBox::textActivated),this,[this](const QString& s){
...
void Game::play() {
    ...
    LOG(INFO) << " before b->figures_start_set();" << (b == nullptr);
    b->figures_start_set();
    LOG(INFO) << "b->figures_start_set();";
    b->show();
    LOG(INFO) << "b->show();\n";
